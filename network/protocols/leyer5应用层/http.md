### HTTP 请求的准备
我们现在以http://www.163.com为例子

浏览器会将 www.163.com 这个域名发送给 DNS 服务器，让它解析为 IP 地址。那接下来是发送 HTTP 请求吗？

不是的，HTTP 是基于 TCP 协议的，当然是要先建立 TCP 连接了，怎么建立呢？这个时候就需要TCP三次握手。
...
![服务器静态资源架构图](../../images/服务器静态资源架构图.jpg)
...
![HTTP1.1与HTTP2请求对比](../../images/HTTP1.1与HTTP2请求对比.jpg)

**HTTP 2.0 其实是将三个请求变成三个流，将数据分成帧，乱序发送到一个 TCP 连接中**。

![HTTP2流和帧传输demo](../../images/HTTP2流和帧传输demo.jpg)
...
这在 TCP 是没有办法的，但是基于 UDP，就可以在 QUIC 自己的逻辑里面维护连接的机制，不再以四元组标识，而是**以一个 64 位的随机数作为 ID 来标识，而且 UDP 是无连接的，所以当 IP 或者端口变化的时候，只要 ID 不变，就不需要重新建立连接**。
...
![QUIC与TCP请求应答对比](../../images/QUIC与TCP请求应答对比.jpg)
